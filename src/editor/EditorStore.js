import {
  applyDefaultNodeVoltageLevels,
  createEdge,
  createNode,
} from "../domain/diagram/createDiagram.js";
import {
  getNodePorts,
...
import {
  assignVoltageToIsland,
  propagateConnectionVoltage,
...
} from "../domain/electrical/parameterValue.js";
import {
  createOperatingCase,
  normalizeOperatingCases,
} from "../domain/analysis/operatingCases.js";
...
function nextNodeName(document, type) {
  ...
  const base = String(
    definition.defaultProperties?.name ||
...
function portForVoltageProperty(node, propertyKey) {
  ...
  return (
    Object.entries(definition.voltagePropertiesByPort).find(
      ([, key]) => key === propertyKey,
    )?.[0] ?? null
  );
}
...
export class EditorStore {
  ...
  setNotice(message, tone = "info") {
    this.setState((state) => ({
      ...state,
      ui: { ...state.ui, notice: message ? { message, tone } : null },
    }));
  }
  ...
  commitDocument(nextDocument, { history = true } = {}) {
    if (this.readOnly) {
      this.setNotice(
        "Este proyecto está disponible sólo para lectura.",
        "info",
      );
      return;
    }
  ...
  createActions() {
    return {
      loadDocument: (document) => {
        if (this.readOnly) {
          this.setNotice(
            "No puedes importar cambios en un proyecto de sólo lectura.",
            "info",
          );
          return;
        }
        ...
        this.future = [];
        this.setState({
          ...createInitialEditorState(document),
      ...
      setTool: (tool) => {
        if (this.readOnly && tool !== "select" && tool !== "pan") {
          this.setNotice(
            "En modo de lectura sólo puedes seleccionar e inspeccionar o mover la vista.",
            "info",
          );
          return;
        }
        this.setState((state) => ({
          ...
          tool,
          connectionDraft:
            DRAW_TOOLS.has(tool) && state.connectionDraft?.kind === tool
      ...
      setRoutingMode: (routingMode) => {
        this.setState((state) => ({
          ...
          connectionDraft: state.connectionDraft
            ? { ...state.connectionDraft, routing: routingMode, vertices: [] }
            : null,
        }));
      },
      ...
      selectNode: (nodeId, mode = "replace") => {
        this.setState((state) => {
          ...
          if (mode === "toggle")
            ...
            nodeIds = current.includes(nodeId) ? current : [...current, nodeId];
          else nodeIds = [nodeId];
          return {
            ...state,
            selection: { nodeIds, edgeId: null, vertexIndex: null },
      ...
      selectVertex: (edgeId, vertexIndex) => {
        this.setState((state) => ({
          ...state,
          selection: { nodeIds: [], edgeId, vertexIndex },
        }));
      },
      ...
      moveNodeTo: (nodeId, position) => {
        ...
        const selected = this.state.selection.nodeIds.includes(nodeId)
      ...
      rotateSelection: (deltaDegrees) => {
        ...
        this.mutateDocument((document) => {
          selected.forEach((id) => {
            ...
            if (node)
              node.rotation = ((node.rotation ?? 0) + deltaDegrees + 360) % 360;
          });
        });
      },
      ...
      setNodeVoltage: (nodeId, propertyKey, levelId) => {
        ...
        this.mutateDocument((document) => {
          ...
          if (portId)
            assignVoltageToIsland(document, { nodeId, portId }, levelId);
          node.properties[propertyKey] = levelId;
          markParameterAsUserValue(node, propertyKey, levelId);
        });
        this.setNotice(
          "Nivel de voltaje propagado hasta el siguiente transformador.",
          "success",
        );
      },
      ...
      updateElectricalParameterMetadata: (kind, id, key, patch) => {
        this.mutateDocument((document) => {
          const entity =
            kind === "node" ? document.nodes[id] : document.edges[id];
      ...
      setActiveVoltageLevel: (levelId) => {
        if (!getVoltageLevel(this.state.document.metadata, levelId)) return;
        this.mutateDocument((document) => {
          document.metadata.activeVoltageLevelId = levelId;
        });
      },
      ...
      createVoltageLevel: (input) => {
        try {
          ...
          this.commitDocument(next);
          this.setNotice(
            `Nivel ${created.label} disponible en el proyecto.`,
            "success",
          );
          return created.id;
        } catch (error) {
      ...
      updateVoltageLevel: (levelId, patch) => {
        try {
          this.mutateDocument((document) => {
            updateVoltageLevel(document, levelId, patch);
          });
      ...
      removeVoltageLevel: (levelId) => {
        const used =
          Object.values(this.state.document.nodes).some((node) =>
            Object.entries(node.properties).some(
              ([key, value]) =>
                key.startsWith("voltageLevelId") && value === levelId,
            ),
          ) ||
          Object.values(this.state.document.edges).some(
            (edge) => edge.properties.voltageLevelId === levelId,
          );
        if (used) {
          this.setNotice(
            "No se puede eliminar un nivel utilizado por componentes o líneas.",
            "error",
          );
          return false;
        }
        try {
          this.mutateDocument((document) =>
            removeVoltageLevel(document, levelId),
          );
      ...
      openVoltageLevels: () => {
        this.setState((state) => ({
          ...state,
          ui: { ...state.ui, voltageLevelsOpen: true },
        }));
      },
      ...
      closeVoltageLevels: () => {
        this.setState((state) => ({
          ...state,
          ui: { ...state.ui, voltageLevelsOpen: false },
        }));
      },
      ...
      openElectricalEditor: (kind, id) => {
        this.setState((state) => ({
          ...state,
          ui: { ...state.ui, electricalEditor: { kind, id } },
        }));
      },
      ...
      closeElectricalEditor: () => {
        this.setState((state) => ({
          ...state,
          ui: { ...state.ui, electricalEditor: null },
        }));
      },
      ...
      addOperatingCase: (name) => {
        let createdId = null;
        this.mutateDocument((document) => {
          const created = createOperatingCase(
            name ||
              `Caso ${normalizeOperatingCases(document.operatingCases).length + 1}`,
          );
          createdId = created.id;
          document.operatingCases = normalizeOperatingCases([
            ...document.operatingCases,
            created,
          ]);
        });
        return createdId;
      ...
      removeOperatingCase: (caseId) => {
        const cases = normalizeOperatingCases(
          this.state.document.operatingCases,
        );
        if (cases.length <= 1) {
          this.setNotice(
            "El diagrama debe conservar al menos un caso de operación.",
            "error",
          );
          return false;
        }
        this.mutateDocument((document) => {
          document.operatingCases = normalizeOperatingCases(
            document.operatingCases.filter((item) => item.id !== caseId),
          );
        });
        return true;
      },
      ...
      updateAnalysisConfiguration: (section, patch) => {
        this.mutateDocument((document) => {
          const current = normalizeAnalysisConfiguration(
            document.analysisConfiguration,
          );
      ...
      setNotice: (message, tone = "info") => this.setNotice(message, tone),
        ...
        this.mutateDocument((document) => {
          ...
          if (nodeIds.length) {
            ...
            Object.values(document.edges).forEach((edge) => {
              if (
                nodeIds.includes(edge.source.nodeId) ||
                nodeIds.includes(edge.target.nodeId)
              )
      ...
      connectDynamicPort: (nodeId, localPoint) => {
        ...
        const projected = definition.dynamicPort.project(
          node,
          snapPoint(
            localPoint,
            this.state.settings.gridSize,
        ...
        const existing = getNodePorts(node).find(
          (port) =>
            Math.hypot(port.x - projected.x, port.y - projected.y) <=
            definition.dynamicPort.reuseDistance,
        );
        if (existing) {
      ...
      addConnectionVertex: (point) => {
        ...
        const anchor =
          draft.vertices.at(-1) ??
          getPortWorldPosition(sourceNode, draft.source.portId);
        const vertex =
          draft.routing === "orthogonal"
            ? orthogonalizeWaypoint(anchor, snapped)
            : snapped;
        this.setState((state) => ({
          ...state,
          connectionDraft: {
            ...state.connectionDraft,
            vertices: [...state.connectionDraft.vertices, vertex],
          },
        }));
      },
      ...
      updateConnectionPointer: (point) => {
        ...
        this.setState((state) => ({
          ...state,
          connectionDraft: { ...state.connectionDraft, pointer: point },
        }));
      },
      ...
      completeConnection: (nodeId, portId) => {
        ...
        if (
          draft.source.nodeId === nodeId &&
          draft.source.portId === String(portId)
        ) {
      ...
      addEdgeVertex: () => {
        ...
        this.mutateDocument((document) => {
          ...
          const insertAt =
            vertexIndex === null ? mutable.vertices.length : vertexIndex + 1;
          const before =
            vertexIndex === null
              ? fullPath.at(-2)
              : mutable.vertices[vertexIndex];
          const after =
            vertexIndex === null
              ? fullPath.at(-1)
              : (mutable.vertices[vertexIndex + 1] ?? fullPath.at(-1));
          mutable.vertices.splice(
            insertAt,
            0,
            snapPoint(
      ...
      deleteEdgeVertex: () => {
        ...
        this.setState((state) => ({
          ...state,
          selection: { ...state.selection, vertexIndex: null },
        }));
      },
      ...
      setEdgeRouting: (edgeId, routing) => {
        this.mutateDocument((document) => {
          const edge = document.edges[edgeId];
          if (!edge) return;
          if (routing === "free" && edge.routing === "orthogonal")
            edge.vertices = getEdgePoints(document, edge).slice(1, -1);
      ...
      setViewport: (viewport) =>
        this.setState((state) => ({ ...state, viewport: { ...viewport } })),
      panViewport: (delta) =>
        this.setState((state) => ({
          ...state,
          viewport: {
      ...
      undo: () => {
        ...
        if (!previous) return;
        this.future = [cloneDiagram(this.state.document), ...this.future].slice(
          0,
          HISTORY_LIMIT,
        );
      ...
      redo: () => {
        ...
        if (!next) return;
        this.past = [...this.past, cloneDiagram(this.state.document)].slice(
          -HISTORY_LIMIT,
        );
  ...
  completeConnectionIntoDocument(nextDocument, nodeId, portId) {
    ...
    if (decision.conflict)
      this.setNotice(
        "Se unificaron dos redes con niveles distintos; prevaleció el nivel de la red más extensa.",
        "warning",
      );
    else
      this.setNotice(
        edge.kind === "line"
          ? "Línea eléctrica creada."
          : "Path visual creado.",
        "success",
      );
  }
